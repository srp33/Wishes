...
with gzip.open(dataOut, 'w') as f :
    ...
    for value in colgrp["id"] :
        f.write(('\t' + geneDict[value.decode('UTF-8')]).encode()) ## Need to cast to str and subset so that it is in the same format as dict
    f.write(('\n').encode())

    index = 0
    for line in subsubgrpData["matrix"] :
        a = np.asarray(line).astype(str)
        f.write((rowgrp["id"][index].decode('UTF-8') + '\t' + '\t'.join(a) + '\n').encode())
        index = index + 1 
        if index % 1000 == 0 :
